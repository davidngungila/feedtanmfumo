...
<head>
    <meta charset="UTF-8">
    <title>{{ $documentTitle ?? 'Loans Report' }}</title>
    <style>
        @page {
            margin: 10mm 12mm;
            size: A4;
...
<body>
    <div class="header">
        ...
        @else
        <div class="title">Loan Portfolio Report</div>
        @endif
        <div class="serial-number">Serial No: FCMGLR-{{ date('Ymd') }}-{{ str_pad(1, 4, '0', STR_PAD_LEFT) }}</div>
    ...
    <div class="footer">
        ...
        <p>Serial No: FCMGLR-{{ date('Ymd') }}-{{ str_pad(1, 4, '0', STR_PAD_LEFT) }}</p>
    </div>
</body>
</html>
