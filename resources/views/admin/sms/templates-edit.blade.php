...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-[#015425] to-[#027a3a] rounded-lg shadow-lg p-6 text-white">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold mb-2">Edit SMS Template</h1>
                <p class="text-white text-opacity-90 text-sm sm:text-base">Update template: {{ $smsMessageTemplate->template_name }}</p>
            </div>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('admin.sms.templates.update', $smsMessageTemplate) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" 
                           id="template_name" 
                           name="template_name" 
                           value="{{ old('template_name', $smsMessageTemplate->template_name) }}"
                           required
                ...
                <div>
                    ...
                    <select id="behavior_type" 
                        ...
                        <option value="Inconsistent Saver" {{ old('behavior_type', $smsMessageTemplate->behavior_type) === 'Inconsistent Saver' ? 'selected' : '' }}>Inconsistent Saver</option>
                        <option value="Sporadic Saver" {{ old('behavior_type', $smsMessageTemplate->behavior_type) === 'Sporadic Saver' ? 'selected' : '' }}>Sporadic Saver</option>
                        <option value="Non-Saver" {{ old('behavior_type', $smsMessageTemplate->behavior_type) === 'Non-Saver' ? 'selected' : '' }}>Non-Saver</option>
                        <option value="Regular Saver" {{ old('behavior_type', $smsMessageTemplate->behavior_type) === 'Regular Saver' ? 'selected' : '' }}>Regular Saver</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea id="message_content" 
                              ...
                              class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#015425] focus:ring-[#015425]">{{ old('message_content', $smsMessageTemplate->message_content) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select id="language" 
                                ...
                                required
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#015425] focus:ring-[#015425]">
                            <option value="sw" {{ old('language', $smsMessageTemplate->language) === 'sw' ? 'selected' : '' }}>Swahili (sw)</option>
                            <option value="en" {{ old('language', $smsMessageTemplate->language) === 'en' ? 'selected' : '' }}>English (en)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" 
                               ...
                               min="1" 
                               max="10" 
                               value="{{ old('priority', $smsMessageTemplate->priority) }}"
                               required
                               class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#015425] focus:ring-[#015425]">
                ...
                <div>
                    ...
                    <input type="text" 
                           id="variables" 
                           name="variables" 
                           value="{{ old('variables', $smsMessageTemplate->variables ? implode(', ', $smsMessageTemplate->variables) : '') }}"
