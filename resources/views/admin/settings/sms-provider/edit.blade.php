...
@section('page-title', 'Edit SMS Provider')
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-[#015425] to-[#027a3a] rounded-lg shadow-lg p-6 text-white">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold mb-2">Edit SMS Provider</h1>
                <p class="text-white text-opacity-90 text-sm sm:text-base">Update SMS gateway provider configuration</p>
            </div>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('admin.sms-provider.update', $smsProvider) }}" method="POST" id="smsProviderForm">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" id="provider_name" value="{{ old('name', $smsProvider->name) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-[#015425] focus:border-[#015425]" placeholder="e.g., Primary SMS Gateway" required>
                            ...
                            <div>
                                ...
                                <input type="text" name="username" id="provider_username" value="{{ old('username', $smsProvider->username) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-[#015425] focus:border-[#015425]" placeholder="SMS gateway username" required>
                                <p class="text-xs text-gray-500 mt-1">SMS gateway username</p>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">SMS Password</label>
                                <input type="password" name="password" id="provider_password" value="" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-[#015425] focus:border-[#015425]" placeholder="Leave blank to keep current password">
                                <p class="text-xs text-gray-500 mt-1">Leave blank to keep current password</p>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">SMS From (Sender Name)</label>
                                <input type="text" name="from" id="provider_from" value="{{ old('from', $smsProvider->from) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-[#015425] focus:border-[#015425]" placeholder="OfisiLink" maxlength="11">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="url" name="api_url" id="provider_api_url" value="{{ old('api_url', $smsProvider->api_url) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-[#015425] focus:border-[#015425]" placeholder="https://messaging-service.co.tz/api/sms" required>
                            </div>

                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                                <textarea name="description" id="provider_description" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-[#015425] focus:border-[#015425]" placeholder="Optional description for this provider">{{ old('description', $smsProvider->description) }}</textarea>
                                <p class="text-xs text-gray-500 mt-1">Optional description for this provider</p>
                            </div>

                            <div>
                                <div class="flex items-center space-x-3">
                                    <input type="checkbox" name="active" value="1" id="provider_active" {{ old('active', $smsProvider->active) ? 'checked' : '' }} class="rounded border-gray-300 text-[#015425] focus:ring-[#015425]">
                            ...
                            <div>
                                <div class="flex items-center space-x-3">
                                    <input type="checkbox" name="is_primary" value="1" id="provider_is_primary" {{ old('is_primary', $smsProvider->is_primary) ? 'checked' : '' }} class="rounded border-gray-300 text-[#015425] focus:ring-[#015425]">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    testBtn.addEventListener('click', function() {
        ...
        const password = document.getElementById('provider_password').value || '{{ $smsProvider->password }}';
        ...
        if (!username || !apiUrl || !testPhone) {
            connectionStatus.className = 'px-4 py-3 bg-red-50 border border-red-300 rounded-md text-sm text-red-800';
            connectionStatus.textContent = 'Please fill in all required fields (Username, API URL, Test Phone)';
            return;
        }
