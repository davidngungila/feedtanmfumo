    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex space-x-2">
                        <button onclick="copyAssessmentLink()" class="inline-flex items-center px-3 py-1.5 bg-orange-100 text-orange-700 rounded-md hover:bg-orange-200 transition text-xs font-bold">
                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/></svg>
                ...
                @php
                    $latestAssessment = $loan->guarantorAssessments()->latest()->first();
                @endphp

                @if($latestAssessment)
                <div class="mb-6 p-4 rounded-xl border {{ $latestAssessment->status === 'approved' ? 'bg-green-50 border-green-100' : ($latestAssessment->status === 'clarification_needed' ? 'bg-orange-50 border-orange-100' : 'bg-blue-50 border-blue-100') }}">
                    <div class="flex justify-between items-center">
                        <div>
                            <span class="block text-xs font-bold text-gray-500 uppercase">Assessment Status</span>
                            <span class="text-sm font-bold {{ $latestAssessment->status === 'approved' ? 'text-green-700' : ($latestAssessment->status === 'clarification_needed' ? 'text-orange-700' : 'text-blue-700') }}">
                                {{ strtoupper(str_replace('_', ' ', $latestAssessment->status)) }}
                            </span>
                        </div>
                        <a href="{{ route('admin.guarantor-assessments.show', $latestAssessment) }}" class="text-xs font-bold underline">Review Assessment &rarr;</a>
                    </div>
                </div>
                @elseif($loan->status === 'pending')
                <div class="mb-6 p-4 rounded-xl bg-gray-50 border border-gray-100 italic text-gray-500 text-sm">
                    No assessment form has been submitted yet for this loan application.
                </div>
                @endif
            ...
            <script>
                function copyAssessmentLink() {
                    const link = "{{ route('guarantor-assessment.show', $loan->ulid) }}";
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="space-y-2">
                    <a href="{{ route('admin.loans.agreement', $loan) }}" target="_blank" class="block w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition text-center font-medium">
