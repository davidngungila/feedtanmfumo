...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
        <form action="{{ route('admin.investments.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1 relative">
                <input type="text" name="search" value="{{ request('search') }}" 
                    placeholder="Search by investment #, member name or code (e.g. ASC108)..." 
            ...
            @if(request('search'))
                <a href="{{ route('admin.investments.index') }}" class="px-6 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition font-medium text-center">
                    Clear
                </a>
            @endif
        </form>
                    ...
                    @forelse($investments as $investment)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <p class="text-[10px] font-bold text-[#015425]">{{ $investment->user->membership_code ?? 'NO CODE' }}</p>
                            </td>
