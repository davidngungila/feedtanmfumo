...
@section('page-title', __('portal.preview_statement'))
...
<div class="max-w-4xl mx-auto space-y-6 pb-12">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('member.monthly-deposits.index') }}" class="inline-flex items-center text-[#015425] font-bold hover:underline">
            ...
            @lang('portal.back_to_statements')
        </a>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ $monthlyDeposit->statement_pdf }}" target="_blank" class="px-4 py-2 bg-orange-600 border-2 border-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 transition-all flex items-center shadow-sm no-print">
                ...
                @lang('portal.download_official_pdf')
            </a>
            @endif
    ...
    <div class="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden" id="printable-area">
        ...
        <div class="p-8 sm:p-12 bg-gradient-to-br from-[#015425] to-[#027a3a] text-white">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h2 class="text-3xl font-extrabold mb-2 uppercase tracking-tighter">@lang('portal.deposit_statements')</h2>
                    <p class="text-white text-opacity-80 font-medium">@lang('portal.monthly_report')</p>
                </div>
        ...
        <div class="p-8 sm:p-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
                <div class="space-y-4">
                    <h4 class="text-xs font-black text-gray-400 uppercase tracking-widest border-b pb-2">@lang('portal.member_details')</h4>
                    <div>
                        <p class="text-sm text-gray-500 mb-1">@lang('portal.full_name')</p>
                        <p class="text-xl font-bold text-gray-900">{{ $monthlyDeposit->name }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500 mb-1">@lang('portal.member_id')</p>
                        <p class="text-lg font-mono font-bold text-[#015425]">{{ $monthlyDeposit->member_id }}</p>
                    </div>
                    @if($monthlyDeposit->email)
                    <div>
                        <p class="text-sm text-gray-500 mb-1">@lang('portal.email_address')</p>
                        <p class="text-sm font-medium text-gray-900">{{ $monthlyDeposit->email }}</p>
                    </div>
                    @endif
                </div>
                <div class="space-y-4 md:text-right">
                    <h4 class="text-xs font-black text-gray-400 uppercase tracking-widest border-b pb-2 md:border-b-0 md:border-b text-right">@lang('portal.issuing_entity')</h4>
            ...
            <h4 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">@lang('portal.contribution_breakdown')</h4>
            <div class="rounded-2xl border border-gray-100 overflow-hidden mb-12">
                <table class="min-w-full">
                    <thead>
                        <tr class="bg-gray-50">
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">@lang('portal.description')</th>
                            <th class="px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">@lang('portal.amount_tzs')</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-gray-700">@lang('portal.monthly_savings_akiba')</td>
                            <td class="px-6 py-4 text-right text-sm font-bold text-gray-900">{{ number_format($monthlyDeposit->savings, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-gray-700">@lang('portal.shares_hisa')</td>
                            <td class="px-6 py-4 text-right text-sm font-bold text-gray-900">{{ number_format($monthlyDeposit->shares, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-gray-700">@lang('portal.welfare_fund')</td>
                            <td class="px-6 py-4 text-right text-sm font-bold text-gray-900">{{ number_format($monthlyDeposit->welfare, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-gray-700">@lang('portal.loan_principal_repayment')</td>
                            <td class="px-6 py-4 text-right text-sm font-bold text-gray-900">{{ number_format($monthlyDeposit->loan_principal, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-gray-700">@lang('portal.loan_interest_repayment')</td>
                        ...
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-red-600">@lang('portal.fines_penalties')</td>
                    ...
                    <tfoot>
                        <tr class="bg-green-50">
                            <td class="px-6 py-6 text-base font-black text-[#015425] uppercase">@lang('portal.total_contribution')</td>
            ...
            <div class="p-8 bg-blue-50 rounded-2xl border border-blue-100 mb-12">
                <h4 class="text-xs font-black text-blue-400 uppercase tracking-widest mb-4">@lang('portal.system_message')</h4>
            ...
            <div class="flex flex-col md:flex-row items-center justify-between pt-12 border-t border-gray-100 gap-6">
                <div class="flex items-center">
                    ...
                    <div>
                        <p class="text-sm font-bold text-gray-900">@lang('portal.digitally_verified')</p>
