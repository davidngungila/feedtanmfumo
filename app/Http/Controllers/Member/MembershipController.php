...
class MembershipController extends Controller
{
    ...
    /**
     * Check if user can edit their application
     */
    private function canEditApplication(User $user): bool
    {
        $allStepsCompleted = $this->allStepsCompleted($user);

        // Can edit if:
        // 1. Not all steps completed yet (still filling out application)
        // 2. OR editing has been requested by reviewer
        if (! $allStepsCompleted) {
            return true;
        }

        return $user->editing_requested === true;
    }
    ...
    public function showStep1()
    {
        ...
        if (! $this->canEditApplication($user)) {
            return redirect()->route('member.membership.preview')
                ->with('error', 'Your application has been submitted. You can only edit if a reviewer requests changes.');
        }
    ...
    public function storeStep10(Request $request)
    {
        ...
        $user->update([
            ...
            'editing_requested' => false, // Clear editing request after resubmission
            'reviewer_comments' => null, // Clear comments after resubmission
        ]);

        $this->markStepCompleted($user, 10);

        // Send thank you email and SMS
        $this->emailService->sendMembershipSubmissionEmail($user);
        $this->smsService->sendMembershipSubmissionSms($user);
